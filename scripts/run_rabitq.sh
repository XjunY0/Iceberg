...
case "$mode" in
  build)
    echo "Building index..."
    ./test/benchmark_rabitq "${BASE_PATH}" "${QUERY_FILE}" "${mode}" "${DATA_DIM}" "${K}" "${efc}" "${M}" "${INDEX_PREFIX_PATH}" "${RESULT_PREFIX_PATH}" "${DATASET_NAME}" "${PROJECT_ROOT}" | tee -a "${log_file}"
    ;;
  search)
